...
@interface STAN_MARG_GTFSDatabase()
...
@implementation STAN_MARG_GTFSDatabase
...
{
    return [[NSBundle mainBundle] pathForResource:@"STAN_MARG_gtfs" ofType:@"db"];
}
...
{
    NSString* pathToDocumentsDirectory = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    return [pathToDocumentsDirectory stringByAppendingPathComponent:@"STAN_MARG_gtfs.db"];
}
