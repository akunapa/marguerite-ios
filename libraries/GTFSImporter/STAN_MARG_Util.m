...
+ (double) ApproximateDistanceWithLat1:(double)lat1 withLon1:(double)lon1 withLat2:(double)lat2 withLon2:(double)lon2
{
    ...
    return [STAN_MARG_Util metersToMiles:(kEarthRadius * (2 * atan2(sqrt(x), sqrt(MAX(0.0, 1.0 - x)))))];
}
...
{
    return [STAN_MARG_Util ApproximateDistanceWithLat1:stop1.coordinate.latitude withLon1:stop1.coordinate.longitude
